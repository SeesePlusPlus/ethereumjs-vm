...
module.exports = function (opts, cb) {
  ...
  const injectNewCodeHandler = (content) => {
    runState.code = new Buffer(content.code, 'hex');
    runState.programCounter = content.pc;

    if ("state" in content) {
      // replace stack, memory, and gasLeft
      runState.stack = content.state.stack;
      runState.memory = content.state.memory;
      runState.gasLeft = content.state.gasLeft;
    }

    preprocessValidJumps(runState);

    opCode = runState.code[runState.programCounter]
    opInfo = lookupOpInfo(opCode)
    opName = opInfo.name
    opFn = opFns[opName]
  ...
  self.on("injectNewCode", injectNewCodeHandler);
  ...
  function parseVmResults (err) {
    ...
    self.removeListener("injectNewCode", injectNewCodeHandler);
    ...
    if (results.exceptionError) {
      ...
      var eventObj = {
        pc: runState.programCounter,
        gasLeft: runState.gasLeft,
        opcode: lookupOpInfo(runState.code[runState.programCounter], true),
        stack: runState.stack,
        depth: runState.depth,
        address: runState.address,
        account: runState.contract,
      ...
      self.emit('exception', eventObj, (content) => {});
    }
